...
#frequency of Activities
freqgen <- table(socialdemo$Gender, exclude=NULL)
freqgen
dim(freqgen)

#proportion of Activities
propgen <- prop.table(freqgen)*100
propgen <- round(propgen, digits=2)
propgen

dim(propgen)

#creating table with binding
tableGen <- cbind(freqgen, propgen)
tableGen <- cbind(Gender = rownames(tableGen), tableGen)
rownames(tableGen) <- NULL
tableGen

#rename column names
colnames(tableGen)[colnames(tableGen) == "freqgen"] <- "Frequency"
colnames(tableGen)[colnames(tableGen) == "propgen"] <- "Proportion (%)"
tableGen

#using kable extra to create better table format
tableGender <- kable(tableGen)%>%
  kable_styling(bootstrap_options = "striped", full_width = F)
